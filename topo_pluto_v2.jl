...
# ╔═╡ fae594be-fa66-4b77-b273-272e74f18d2e
using Plots, PlutoUI, Images, DataFrames, LaTeXStrings

# ╔═╡ b4abdb6d-0dca-44bc-a0e9-61abb75211b2
using DifferentialEquations, StaticArrays

# ╔═╡ bb24151a-84d7-4380-9e36-341431b12efd
html"""
<style>
  main {
		margin: 0 auto;
...
# ╔═╡ 1408940c-8e0c-11ed-127a-535f4751c84c
html"<button onclick='present()'>present</button>"

# ╔═╡ 470433e9-1971-41aa-9211-2a4775e7b859
md"""
# Hands on `Pluto`
### Dynamical systems with reactive `Julia` notebooks
"""

# ╔═╡ 2464849a-a704-460f-9806-67ea64b27eb7
Resource("https://upload.wikimedia.org/wikipedia/commons/thumb/e/ef/Pluto_in_True_Color_-_High-Res.jpg/800px-Pluto_in_True_Color_-_High-Res.jpg",
:width => 400)

# ╔═╡ af8d2f9d-78bb-4292-9ffa-888f279d9a76
md" Ludovic Mailleret, M2P2-ISA (INRAE, CNRS, UCA) & Biocore Inria; [ludovic.mailleret@inrae.fr](ludovic.mailleret@inrae.fr)"

# ╔═╡ e5720e24-989d-40dd-a2ac-db39df88fde2
md"# What is `Pluto`?"

# ╔═╡ 74962ed3-7f8a-4c26-bd4f-5de2bafcf9da
md"""
## `Pluto` is a notebook solution for `Julia`
"""

# ╔═╡ 934c7a45-61d4-4eac-a5df-81b65293fc6a
md"- You get cells to put code or markdown/LaTeX notes"

# ╔═╡ 0dcc268e-5b62-4a32-b8a8-e3fb99a19abb
md"- Code is executed and rendered within the environment (data/plots exports are possible)"

# ╔═╡ 83df6c49-e15b-4302-9643-5245812feb52
md"- Nice for exploring models, sandbox, code sharing, supplementary materials... 
  - or making interactive presentations like the present one (we are actually in a `Pluto` notebook)"

# ╔═╡ c62354d7-5e06-4c92-b3df-cb42344d681e
md"- `Pluto` was developed for the free MIT course [Introduction to Computational Thinking](https://computationalthinking.mit.edu/) (which is very nice!)"

# ╔═╡ 24b3b7ae-3330-47df-97dc-6c1ea7d80697
md"- `Pluto` is written in `Julia`, for `Julia` coding"

# ╔═╡ 52d072f9-8fbb-4d1c-a509-6d1e5063ff62
md"- `Pluto` is *reactive*"

# ╔═╡ f1ae85d5-eb89-4b6e-854f-509777a3d1c3
md"## Pluto is a reactive Julia notebook environment"

# ╔═╡ f535cb57-3b8c-4365-916f-7fd2ff009d74
md"- Each cell is **always executed** in the `workspace` or `scope`
- Therefore **dependent cells react to changes** "

# ╔═╡ 1385b381-31db-42e4-ad88-d1a32a0ea319
k = 1;

# ╔═╡ b95f09c4-3ba6-42d0-91d5-9355180eb7d7
md"
## Reactivity is handy
"

# ╔═╡ 6080d186-5472-443b-925d-b655af7e4ebe
num_cats = 3;

# ╔═╡ bf64392d-ff5b-478e-8bce-9765256af3f4
md"- **I have $num_cats cats**"

# ╔═╡ 70d7545d-b197-47be-922a-f983c22618cb
md"""
- But of course **you can't do anything** ($\neq$ e.g. Jupyter, or classical scripting)
"""

# ╔═╡ 90b1d8de-6fed-48fc-8054-5c55c3d9c277
foo = 2;

# ╔═╡ de2b1957-c68a-47e4-ab1c-319830fa96df
# foo = 4;

# ╔═╡ 52a9fd86-4d39-4df6-b61c-57609707122f
md"- Multi lines of code should be put in: `begin ... end` blocks (variable update possible in blocks)"

# ╔═╡ c9343502-84a5-4f2a-b7d1-c2b8b450d746
begin
	pi = π
	cos(pi), sin(pi)
end

# ╔═╡ a43d8586-49d8-4bab-812b-f5b6764ff538
md"- but not for function definition, or `if, for, while` blocks (variable update possible in blocks)"

# ╔═╡ 858523bd-9ba0-4686-9a29-a320b8981882
md"## Reactivity is handy"

# ╔═╡ d3d0e6e4-1d3d-4834-95df-e6bef3152926
md"n = $(@bind n Slider(1:10, show_value=true, default=.1))"

# ╔═╡ 529c759e-5d4f-4f63-ac1a-bd76b98c4c6a
md"# Simulating differential equations"

# ╔═╡ e9e7e809-443d-473a-a4f4-695eef7b9098
md"## Simulating ODEs with `DifferentialEquations.jl`"

# ╔═╡ 4f4caddd-0b33-4b26-bc61-ba14f7b2f3e0
md"
Consider the predator-prey model attributed to [Rosenzweig & MacArthur (1963)](http://www.jstor.com/stable/2458702) (see [Turchin (2003)](http://www.jstor.com/stable/2458702), [Smith (2008)](https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.710.95&rep=rep1&type=pdf)).
...
# ╔═╡ dede1df6-eb74-4bf6-a0fa-18aefd2e5840
md"""
...
# ╔═╡ 9c88c0da-09bf-4bc6-9ecf-618fad4a6253
...
# ╔═╡ 638434a6-a0a8-4b26-aa3d-610628f1367a
md"""
## Initial conditions, parameters & time
"""

# ╔═╡ 380f7884-44f2-4905-a61d-b07c2736156e
md"- Initial conditions"

# ╔═╡ cb65d582-c4b8-4326-8b72-02a81c95019b
begin
	x0 = 1.0
	y0 = 2.5
...
# ╔═╡ f2e5cef0-497d-48ee-8e1a-e0b6e32db79f
md"- Parameters"

# ╔═╡ dbee8d7a-e743-4135-b790-9cb2af461bc5
md"- Integration time"

# ╔═╡ 183c38d1-a4a5-4064-bc90-3cacdbeb9e01
begin
	tspan = (0.0, 80.0) 	# (start, end) as a tuple
	step = 0.01
end;

# ╔═╡ 550b6916-6562-450a-9921-8e1fedc5bc1e
md"## Numerical integration"

# ╔═╡ 71f3855f-2bf2-4af7-b969-8f2b8dadb1fc
md"- Define the Cauchy problem"

# ╔═╡ b3e0aa01-dc3c-4a5a-a817-3a2db4fd6810
md"- Integrate"

# ╔═╡ a94ac297-0e12-4387-b0ae-0d4784ae9105
md"- Rearrange the simulation in a `dataframe`, rename data (optional)"

# ╔═╡ c63db042-831b-44fb-ac88-526313709be5
md"## Numerical integration
- you get the simulated solution along time, every $step timesteps"

# ╔═╡ 3cca65f0-2122-400f-8d2f-980b43e710b3
md"## Plotting against time"

# ╔═╡ 92013ea4-57dd-4f5f-8929-3717f2ffdac6
md"## Plotting in state space"

# ╔═╡ 46331895-b1f1-41a4-a508-3d5208aeda71
md"- One can have nice state space plots, but code is longer"

# ╔═╡ d293a064-38fb-4712-bc34-6c7df194d7ab
md"## Playing with plots in the state space"

# ╔═╡ cb35beec-86ee-467d-aee6-5fedd42945fb
md"m =  $(@bind m Slider(0.6:.1:1.5, show_value = true, default = 1.0))"

# ╔═╡ 84a13da2-3ae5-4d8d-b012-ad6188ee8033
md"# Bifurcation diagram"

# ╔═╡ 5259e740-8577-417b-9e2a-d69fc52d0315
md"""## Bifurcation diagram in function of $K$
...
# ╔═╡ 43d68ca4-8527-4402-9a6d-6a78f4286379
md"""
## Bifurcation diagram in function of $K$
- Simulate transients, restart from there, and get extrema
"""

# ╔═╡ 77eb1313-9312-42e0-91ba-a6f3e65e6c34
md"""## Bifurcation diagram in function of $K$
- After plotting everything"""

# ╔═╡ a140bfdd-2b0c-4a06-8941-2f8183ddbc67
md"""## Playing with bifurcation diagrams
h =  $(@bind h Slider(1.:.2:2.5, show_value = true, default = 2.0))
"""

# ╔═╡ d001019a-93fb-4efc-97cc-1bb46f9d0678
begin
	r = 1.0 
	K = 10.0
	c = 1.0
...
# ╔═╡ a6983a6a-76d3-4a25-b01a-a2b4021e476c
prob_rma = ODEProblem(model_rma, etat0, tspan, params_rma, saveat = step)

# ╔═╡ d04ac684-26d2-4ef3-80c5-18fd6169584b
sim_rma = solve(prob_rma, abstol=1e-6, reltol=1e-6);

# ╔═╡ d582bb6a-73f2-4feb-885b-5dfd3664e88d
begin
	sol_rma = DataFrame(sim_rma)
	rename!(sol_rma, :timestamp => :time, :value1 => :x, :value2 => :y)
end;

# ╔═╡ 9883cac2-2cfe-451f-91be-fa801f8b199c
sol_rma

# ╔═╡ 52a74c9d-82d2-4c57-bd9e-e4c5ec2c4965
plot(sol_rma.time, [sol_rma.x sol_rma.y], 
	palette = :tab10,
	linewidth = 2,
	title = "Rosenzweig MacArthur model",
	label = ["prey " * L"x" "predators " * L"y"],  # latex strings, mardown latex is off in labels
	ylabel = "population densities",
	xlabel = "time",
	size = (650,350))

# ╔═╡ ed4f4731-0744-41e0-8ebb-8ee5a77e7072
 begin
 	K_step = 0.1
...
# ╔═╡ 1e7ab0de-fa86-45dc-b4de-573a126cb3a1
md"## Final words"

# ╔═╡ 71e9c76e-f1aa-4a60-a959-ca9651a501a5
md"""
**Pros:**
...
# ╔═╡ 0ace25fc-b917-449d-af51-90593a73bf60
md"""
**Cons:**
...
# ╔═╡ b563f760-b02d-4eaf-848f-6298f9a39933
md"## "

# ╔═╡ 4907e125-1794-47b9-9855-513607feb7ff
md"## additional stuff"

# ╔═╡ f2a30943-94a5-4c72-aa07-b3c8cc85e580
meshgrid(x, y) = (repeat(x, outer=length(y)), repeat(y, inner=length(x)))

# ╔═╡ a895b566-c193-4e0f-be3b-94f794658b2a
meshx, meshy = meshgrid(0:.7:K+.1, 0:.7:K+.1);

# ╔═╡ c63653c9-7dec-4a43-b15c-012059114ae9
der_x(x, y) = model_rma([x y], params_rma, 0)[1]/7

# ╔═╡ 5b472cd2-12c7-4fd9-9dc0-003ee352e947
der_y(x, y) = model_rma([x y], params_rma, 0)[2]/7

# ╔═╡ e8b6328c-dcca-4778-a97e-130e81d01e6d
begin
	# vector field
	...
	# trajectory
	plot!(figure2, sol_rma.x, sol_rma.y,
		palette = :tab10,
		color = 1,
		lw = 2,
		label = "trajectory")
...
# ╔═╡ c6e86553-a489-4b5d-b1fc-3bdce4a800dd
figure2

# ╔═╡ 524c65b6-520c-4cd5-8e4e-16b492a886f8


# ╔═╡ 93db92c2-282c-42b4-a711-e69869bfd8a7
Vector{Float64}(undef, 10)

# ╔═╡ c8fb4b6d-105a-4f01-8852-3aaf77020ef7
x = 0:.001:1

# ╔═╡ fc200ad4-6d1a-4c85-9771-b97d8fe89150
ySin = sin.(2π * k * x) ;

# ╔═╡ 68f3934b-d8bb-444f-9562-1d35ede3a64c
plot(x, ySin, 
	linewidth = 2,
	label = "sin(2π a x)",
	title = "Plot example",
...
# ╔═╡ ded74e69-f733-4f0f-9309-f9f5af4ed12c
yCos = cos.(2π * n * x);

# ╔═╡ efb65936-fa7c-4d51-9b56-589caaf835b0
plot(x, yCos,
	linewidth = 2,
	label = "cos(2πnx)",
...
# ╔═╡ 9cc2d7c6-4aa3-4648-abcd-7786151b3dd5
t_trans = (0.0, 8000);

# ╔═╡ 42bd2958-cdb3-4680-b7b0-916efadc0581
begin
	# for storage
	i = 1
	y_cmin = zero(K_plot3)
...
# ╔═╡ 9b128897-af3b-4f6d-b226-de28800d80d7
begin
...
# ╔═╡ 6e83aad4-f040-4ee7-b473-6e9f22ab6635
figure3
...
julia_version = "1.8.4"
manifest_format = "2.0"
project_hash = "e5bd0922dcbfff16cdaf57c9a96a02213c082636"
...
git-tree-sha1 = "91abc7c0e436e851503da30e927b27ef683ee080"
